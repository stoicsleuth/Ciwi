...
const titleFetcher = (url, query, orderBy = 'Date', countryFilter = [ 337 ], offset = 0) => {
  if (!url) return null

  const requestUrl = `${url}&orderby=${orderBy}&countrylist=${countryFilter.join(',')}&country_andorunique=and&offset=${offset || 0}&query=${query || ''}`
